...
export async function getSushiV2CandidatePools({
  tokenIn,
  tokenOut,
  routeType,
  routingConfig,
  ...
  chainId,
}: SushiV2GetCandidatePoolsParams): Promise<{
  ...
  const poolAccessor = await poolProvider.getPools(
    uniTokenPairToSushiTokenPairs(tokenPairs),
    {
      blockNumber,
    }
  );
